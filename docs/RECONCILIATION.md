...
| `bosd/feat/data-quality-module-final` + `fluvo-data-quality-dashboard` | 17 | Full Odoo addon `modules/fluvo_data_quality_dashboard/` (models, security, views, scheduled actions, readme) | **No** — not a Python-library concern; never present | **KEEP AS SEPARATE MODULE** — strongest open-core/premium candidate; home decided with monetization |
...
1. In `_plan_deferrals_and_strategies` (`src/fluvo/lib/preflight.py:480`), extend the
...
2. Add the safeguard from the fork to `src/fluvo/importer.py`: if a user explicitly passes a
...
`modules/fluvo_data_quality_dashboard/` is a self-contained OCA-style addon (own models/security/views/
cron). It does not belong in the `fluvo` Python package and must not block the library release.
