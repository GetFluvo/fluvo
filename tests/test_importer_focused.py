...
def test_importer_main_process_with_relational_strategies():
                ...
                with patch("odoo_data_flow.importer.import_threaded.import_data") as mock_import:
                    ...
                    with patch("odoo_data_flow.importer.relational_import.run_direct_relational_import") as mock_rel_import:
                        ...
                        with patch("odoo_data_flow.importer.pl.read_csv") as mock_read_csv:
                            ...
                            mock_df = pl.DataFrame({"id": ["1", "2"], "name": ["Alice", "Bob"], "tags": ["tag1,tag2", "tag3,tag4"]})
...
def test_importer_with_write_tuple_strategy():
            ...
            with patch("odoo_data_flow.importer._run_preflight_checks") as mock_preflight:
                ...
                with patch("odoo_data_flow.importer.import_threaded.import_data") as mock_import:
                    ...
                    with patch("odoo_data_flow.importer.relational_import.run_write_tuple_import") as mock_write_tuple:
                        ...
                        import polars as pl
                        with patch("odoo_data_flow.importer.pl.read_csv") as mock_read_csv:
                            mock_df = pl.DataFrame({"id": ["1", "2"], "name": ["Alice", "Bob"], "parent_id": [101, 102]})
                            mock_read_csv.return_value = mock_df
...
def test_importer_with_write_o2m_tuple_strategy():
            ...
            with patch("odoo_data_flow.importer._run_preflight_checks") as mock_preflight:
                ...
                with patch("odoo_data_flow.importer.import_threaded.import_data") as mock_import:
                    ...
                    with patch("odoo_data_flow.importer.relational_import.run_write_o2m_tuple_import") as mock_write_o2m:
                        ...
                        import polars as pl
                        with patch("odoo_data_flow.importer.pl.read_csv") as mock_read_csv:
                            mock_df = pl.DataFrame({"id": ["1", "2"], "name": ["Alice", "Bob"], "child_ids": ["101,102", "103,104"]})
                            mock_read_csv.return_value = mock_df
...
def test_importer_with_write_tuple_failure():
