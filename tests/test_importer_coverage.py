...
def test_import_data_with_all_features():
    ...
    try:
        with patch("odoo_data_flow.import_threaded._read_data_file") as mock_read:
            mock_read.return_value = (["id", "name", "category_id"], [["1", "Alice", "cat1"], ["2", "Bob", "cat2"]])
            
            with patch("odoo_data_flow.lib.conf_lib.get_connection_from_dict") as mock_get_conn:
                mock_connection = MagicMock()
                mock_get_conn.return_value = mock_connection
                mock_model = MagicMock()
                mock_connection.get_model.return_value = mock_model
                
                with patch("odoo_data_flow.import_threaded._orchestrate_pass_1") as mock_pass_1:
                    mock_pass_1.return_value = {"success": True, "id_map": {"1": 101, "2": 102}}
                    
                    with patch("odoo_data_flow.import_threaded._orchestrate_pass_2") as mock_pass_2:
...
def test_importer_with_all_options():
    ...
    try:
        with patch("odoo_data_flow.importer._count_lines", return_value=3):
            with patch("odoo_data_flow.importer._run_preflight_checks", return_value=True):
                with patch("odoo_data_flow.importer.import_threaded.import_data") as mock_import:
                    mock_import.return_value = (True, {"id_map": {"1": 101, "2": 102}, "total_records": 2})
                    
                    # Mock polars reading that works correctly
                    with patch("odoo_data_flow.importer.pl.read_csv") as mock_read_csv:
                        import polars as pl
                        mock_df = pl.DataFrame({"id": ["1", "2"], "name": ["Alice", "Bob"]})
...
def test_importer_edge_cases():
    ...
    try:
        with patch("odoo_data_flow.importer._count_lines", return_value=0):  # No records to retry
            with patch("odoo_data_flow.importer.Path") as mock_path:
                ...
                with patch("odoo_data_flow.importer.Console") as mock_console:
...
def test_importer_csv_reading_fallbacks():
            ...
            with patch("odoo_data_flow.importer._run_preflight_checks", return_value=True):
                with patch("odoo_data_flow.importer.import_threaded.import_data") as mock_import:
                    ...
                    with patch("odoo_data_flow.importer.pl.read_csv") as mock_read_csv:
                        ...
                        mock_header_df = pl.DataFrame([["id", "name"]], schema={"column_1": pl.Utf8, "column_2": pl.Utf8})
                        # Simpler approach - just mock the method to return the expected DataFrame
                        mock_df = pl.DataFrame({"id": ["1"], "name": ["Alice"]})
                        mock_read_csv.return_value = mock_df
